				...
				<div class="card-body" >
					...
					<form name="form1" method="POST" action="{{ route('calculation.index') }}" style="align-items:center;">
						...
						<!-- This script updates the hidden inputs further below to match the inputs the user changes -->
							<script>
								function update_var(element) {
									...
									formFunctionName = "method_id_for_form";
									document.getElementById(formFunctionName.concat(methodValue)).checked = element.checked;
								}
						...
						@php
							...
							for($i=1;$i<$count+1;$i++){
								echo '<input style="display:none" id="method_id_for_form'.$i.'" name="method_id_for_form'.$i.'" type="checkbox" value="'.$i.'">';
							}
